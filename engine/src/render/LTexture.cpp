...
bool LTexture::loadTextureFromFile(const char * path )
{
    ...
    int width, height, nmc;
    unsigned char* image = stbi_load(path, &width, &height, &nmc, 0);
    GLboolean success = GlLoadImage( path );

    //Image loaded successfully
    if( success == GL_TRUE )
    {
        textureLoaded = loadTextureFromPixels32( (GLuint*)image, (GLuint)width, (GLuint)height);
...
bool LTexture::loadTextureFromPixels32( GLuint* pixels, GLuint width, GLuint height, int nmc)
{
    ...
    //Generate texture
    glTexImage2D( GL_TEXTURE_2D, 0, nmc == 3 ? GL_RGB : GL_RGBA, width, height, 0, nmc == 3 ? GL_RGB : GL_RGBA, GL_UNSIGNED_BYTE, pixels );
